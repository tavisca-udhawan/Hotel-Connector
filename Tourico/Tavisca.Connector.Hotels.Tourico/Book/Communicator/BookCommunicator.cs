...
{
    public class BookCommunicator
    {
        private readonly IHttpConnector _connector;

        public BookCommunicator(IHttpConnector connector)
        {
            _connector = connector;
        }

        internal async Task<HttpResponse> BookAsync(SupplierBookingRq supplierRequest, Supplier supplier, SupplierConfiguration supplierConfiguration)
        {
            using (var profileScope = new ProfileContext("Tourico-ConnectorCall", false))
            {
                ...
                var httpRequest = Http.NewPostRequest(new Uri(supplierConfiguration.BookingUrl), settings)
                    ...
                    .WithHttpFilter(new Common.WebCaller.LoggingHttpFilter(true))
                    .WithHttpFilter(new ResponseDataExtractorHttpFilter<SupplierBookingRs, ErrorTypes>(new DataLogger().ExtractDataFromResponse))
                    .WithApiLogData(SupplierConstants.BookConstants.Api, SupplierConstants.BookConstants.Verb, SupplierConstants.BookConstants.MethodName, supplierConfiguration)
