...
{
    public class CancelCommunicator
    {
        private readonly IHttpConnector _connector;

        public CancelCommunicator(IHttpConnector connector)
        {
            _connector = connector;
        }

        internal async Task<HttpResponse> CancelAsync(SupplierCancelRq supplierRequest, Supplier supplier, SupplierConfiguration supplierConfiguration)
        {
            using (var profileScope = new ProfileContext("Tourico-ConnectorCall", false))
            {
                ...
                var httpRequest = Http.NewPostRequest(new Uri(supplierConfiguration.CancelUrl), settings)
                    ...
                    .WithHttpFilter(new ResponseDataExtractorHttpFilter<SupplierCancelRs, ErrorTypes>(new DataLogger().ExtractDataFromResponse))
                    .WithApiLogData(SupplierConstants.CancelConstants.Api, SupplierConstants.CancelConstants.Verb, SupplierConstants.CancelConstants.MethodName, supplierConfiguration)
