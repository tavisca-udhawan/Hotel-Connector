...
{
    public class RetrieveCommunicator
    {
        private readonly IHttpConnector _connector;

        public RetrieveCommunicator(IHttpConnector connector)
        {
            _connector = connector;
        }

        internal async Task<HttpResponse> GetRetrieveAsync(SupplierRetrieveRq supplierRequest, Supplier supplier, SupplierConfiguration supplierConfiguration)
        {
            using (var profileScope = new ProfileContext("Tourico-ConnectorCall", false))
            {
                ...
                var httpRequest = Http.NewPostRequest(new Uri(supplierConfiguration.RetrieveUrl), settings)
                    ...
                    .WithHttpFilter(new ResponseDataExtractorHttpFilter<SupplierRetrieveRs, ErrorTypes>(new DataLogger().ExtractDataFromResponse))
                    .WithApiLogData(SupplierConstants.RetrieveConstants.Api, SupplierConstants.RetrieveConstants.Verb, SupplierConstants.RetrieveConstants.MethodName, supplierConfiguration)
