...
{
    public class RoomRatesCommunicator
    {
        private readonly ISerializer _serializer;
        private readonly IHttpConnector _connector;

        public RoomRatesCommunicator(IHttpConnector connector)
        {
            _connector = connector;
        }

        internal async Task<HttpResponse> GetRoomRatesAsync(SupplierRoomRatesRq supplierRequest, Supplier supplier,  SupplierConfiguration supplierConfiguration)
        {
            using (var profileScope = new ProfileContext("Tourico-ConnectorCall", false))
            {
                ...
                var httpRequest = Http.NewPostRequest(new Uri(supplierConfiguration.RoomRatesUrl), settings)
                    ...
                    .WithHttpFilter(new ResponseDataExtractorHttpFilter<SupplierRoomRatesRs, ErrorTypes>(new DataLogger().ExtractDataFromResponse))
                    .WithApiLogData(SupplierConstants.RoomRatesConstants.Api, SupplierConstants.RoomRatesConstants.Verb, SupplierConstants.RoomRatesConstants.MethodName, supplierConfiguration)
